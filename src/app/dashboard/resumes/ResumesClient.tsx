...
export default function ResumesClient({ profile }: Props) {
  ...
  const [viewing,   setViewing]   = useState<string | null>(null) // resume id being opened
  ...
  async function handleUpload() {
    ...
    if (!label.trim()) {
      toast.error('Add a label for this resume')
      return
    }
  ...
  async function handleView(resumeId: string, filePath: string | null) {
    if (!filePath) {
      toast.error('No file attached to this resume')
      return
    }
    ...
    <div className="flex-1 overflow-y-auto p-6 max-w-3xl mx-auto w-full">
      ...
      <div className="mb-6">
        ...
        <p className="text-[14px] text-slate-400">
          Upload different resume versions and track which one gets the most callbacks.
        </p>
      </div>
          ...
          <div className="flex flex-col gap-3">
            {resumes.map((resume) => {
              ...
              return (
                <div
                  key={resume.id}
                  className="bg-white rounded-2xl border border-slate-200 p-4 flex items-center gap-4 hover:border-slate-300 transition-colors"
                >
                  {/* Icon */}
